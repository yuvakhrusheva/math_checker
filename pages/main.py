"""Main screen (Cohort Queue) — v6 (stage16).

stage16 — ОПТИМИЗАЦИЯ:
- Список когорт рендерится из ОДНОГО запроса через
  db.get_cohort_overview_for_user (вместо N×3 = 200+ запросов при 68
  когортах). Это главный источник «тормозов» главной страницы.
- Блок «работы с проблемами» по-прежнему доступен по кнопке-разворачивалке —
  но загружает детали ЛЕНИВО (при разворачивании expander), а не на каждый
  рендер всей страницы.
- Live Progress тоже использует overview, поэтому полная цена страницы =
...
def _render_cohort_table(overview_rows):
    """stage16: рисуем когорты из ПРЕДагрегированных данных (один запрос).

    overview_rows — список dict'ов от db.get_cohort_overview_for_user.
    """
    if not overview_rows:
        st.info("У тебя пока нет когорт. Импортируй их через форму ниже.")
        return

    for cohort in overview_rows:
        total = cohort["total"]
        processed = cohort["processed_or_terminal"]
        n_unfinished = cohort["n_unfinished"]
        n_failed = cohort["n_failed"]
        ...
        with cols[7]:
            ...
            if n_unfinished > 0 and not queue_processor.is_running():
                if st.button("🔄", key=f"retry_{cohort['id']}",
                    ...
                    st.success(f"{n} работ сброшено в очередь.")
                    st.rerun()

        # Список «работ с проблемами» — рендерим только когда expander открыт.
        # st.expander сам по себе ничего не вычисляет, пока не развернут;
        # детали загружаем лениво по запросу пользователя.
        if n_failed:
            with st.expander(f"⚠️ {n_failed} работ с проблемами"):
...
def _render_root_import_form():
    with st.expander("📁 Import from root folder (auto-detect cohorts)", expanded=True):
        st.caption(
            ...
            "**Шаг 2.** Выбери, что именно ты хочешь проверять."
        )
        ...
        with browse_col:
            if st.button("🔍 Browse folder structure"):
                with st.spinner("Walking Drive folder structure (может занять до минуты)…"):
        ...
        st.caption(
            f"**Selection:** {len(after_t)} teacher folders match. "
            "При большом импорте подожди до 1-2 минут — это нормально."
        )
...
def _render_progress(overview_rows):
    processing = [c for c in overview_rows if c["status"] == "processing"]
    if not processing:
    ...
    for cohort in processing:
        ...
        if total == 0:
            continue
        done = cohort["processed_or_terminal"]
        label = f"{cohort['school']} — {cohort['class_number']}{cohort['class_letter']}"
        st.write(f"**{label}** — {done}/{total} done | {cohort['n_unfinished']} pending")
...
# ОДИН SQL-запрос на всё, что нужно для рендера таблицы и Live Progress.
overview = db.get_cohort_overview_for_user(_CU.get("id"), _CU.get("role"))
# Soft-deleted (in_project=0) убираем из таблицы.
visible = [c for c in overview if c.get("in_project")]
